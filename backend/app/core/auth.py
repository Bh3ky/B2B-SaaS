...
class AuthUser:
    ...
    # function to check if user has a specific permission inside the org list
...
async def get_current_user(request: Request) -> AuthUser:
    ...
    # inside the payload, we have the user ID and org info
    claims = request_state.payload
    user_id = claims.get("sub")
    org_id = claims.get("org_id")
    org_permissions = claims.get("permissions") or claims.get("org_permissions") or []

    # validate presence of user_id and org_id
    if not user_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Not authenticated"
        )
    
    # validate presence of org_id
    if not org_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No organisation selected"
        )
